...
namespace App_UI.ViewModels
{
    public class UsersViewModel : BaseViewModel
    {
        ...
        private void initRegEx()
        {
            ...
            RePhoneNumber = new Regex(@"^[\(]?\d{3}[ \-)]?[- ]?\d{3}[ -]?\d{4}$");
            RePostalCode = new Regex(@"^[a-zA-Z0-9]\d[a-zA-Z0-9][ -]?\d[a-zA-Z0-9]\d?$");
        ...
        private void ValidateData(string obj)
        {
            ...
            if (ReProvince.IsMatch(Province))
            {
                ...
                {
                    Message += "Le téléphone n'a pas le bon format." + Environment.NewLine;
                }
            }
            else
            {
